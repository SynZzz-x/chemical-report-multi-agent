...
8. 每个任务必须且只能包含 14 个 Task Contract 字段，不得增加或省略字段；`covers_sections` 必须逐字引用建议章节中 `kind=content` 的 section。
...
18. `depends_on_task_ids` 必须显式表达真实执行依赖并保持无环；独立任务使用 `[]`，synthesis 列出其消费的全部前置任务。临时 task_id 在提交时会被稳定重映射，dependency 必须跟随映射。
